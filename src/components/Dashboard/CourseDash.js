...
const CourseDash = () => {
  ...
  const handleUpdate = (course) => {
    ...
    setSelectedInstructorId(course.instructor); // Ensure selectedInstructorId is set correctly
    setShowModal(true);
  };
  ...
  const handleUpdateCourse = async (e) => {
    ...
    try {
      ...
      const response = await Api.put(`/api/courses/${selectedCourse._id}`, {
        ...newCourse,
        instructor: selectedInstructorId // Ensure this is just the instructor ID
      });
      const updatedCourse = response.data.Course;

      // Fetch the updated instructor details
      const instructorResponse = await Api.get(`/api/users/${updatedCourse.instructor}`);
      const updatedInstructor = instructorResponse.data;

      // Merge the updated instructor details into the updated course object
      const updatedCourseWithInstructor = {
        ...updatedCourse,
        instructor: updatedInstructor
      };

      const updatedCourses = courses.map((course) =>
        course._id === updatedCourseWithInstructor._id ? updatedCourseWithInstructor : course
      );
      setCourses(updatedCourses);
      console.log('Course updated:', updatedCourseWithInstructor);
      closeModal();
      Swal.fire('Success', 'Course updated successfully', 'success');
    } catch (error) {
      console.error('Error updating course:', error.response.data.details);
      ...
      <Modal className='dash-model' show={showModal} onHide={closeModal}>
        ...
        <Modal.Body>
          <Form onSubmit={selectedCourse ? handleUpdateCourse : handleSubmit}>
            ...
            <Form.Group controlId="formDescription">
              <Form.Label className='modal-data' >Description</Form.Label>
              <Form.Control as="textarea" name="description" value={newCourse.description} onChange={handleChange} />
            </Form.Group>
            <Form.Group controlId="formType">
              <Form.Label className='modal-data' >Type</Form.Label>
              <Form.Control type="text" name="type" value={newCourse.type} onChange={handleChange} />
            ...
            <Form.Group controlId="formWillLearn">
              <Form.Label className='modal-data' >Will Learn</Form.Label>
              <Form.Control as="textarea" name="willLearn" value={newCourse.willLearn} onChange={handleChange} />
            </Form.Group>
            <Form.Group controlId="formRequirementR1">
              <Form.Label className='modal-data' >Requirement 1</Form.Label>
              <Form.Control type="text" name="requirement.r1" value={newCourse.requirement.r1} onChange={handleChange} />
            </Form.Group>
            <Form.Group controlId="formRequirementR2">
              <Form.Label className='modal-data' >Requirement 2</Form.Label>
              <Form.Control type="text" name="requirement.r2" value={newCourse.requirement.r2} onChange={handleChange} />
            </Form.Group>
            <Button className='submit-btn-modal' type="submit">
            {isLoading ? 'Updating...' : selectedCourse ? "Update Course" : "Add Course"}
          </Button>
          </Form>
        </Modal.Body>
      </Modal>
